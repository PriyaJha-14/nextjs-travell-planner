...
const Search = () => {
    ...
    const searchCities = async (searchQuery: string) => {
        const response = await fetch(
            `https://secure.geonames.org/searchJSON?q=${searchQuery}&maxRows=5&username=priya&style=SHORT`
        );
        const parsed = await response.json();

        // ✅ safely typed geonames array
        const names: string[] = (parsed?.geonames as { name: string }[] | undefined)
            ?.map((city) => city.name) ?? [];
            ...
            <div className="absolute h-[50vh] w-[60vw] flex flex-col gap-5">
                ...
                <div className="grid grid-cols-3 gap-5  p-5 rounded-xl ">
                    ...
                    <Input
                        color="danger"
                        variant="bordered"
                        className="text-white placeholder:text-white relative border border-white/50 rounded-md"
                    ...
                    {/* 🔹 Date input with border */}
                    <Input
                        type="date"
                        placeholder="Dates"
                        variant="bordered"
                        color="danger"
                        className="text-white accent-danger-500 border border-white/50 rounded-md"
                    ...
                    <Button
                        size="lg"
                        className="h-full cursor-pointer bg-pink-500 hover:bg-pink-600 text-white shadow-lg"
                        color="default" // keep default so Tailwind class controls color
                        variant="shadow"
